...
bambooSpecsDirectory="$1"
  ...
  cd harvesterSetupTemp/bamboo-specs/plans

  # create Bamboo plans
  RunBambooSpecs "$atlassianUserName" "$atlassianPassword"

  # add plan branches
  planName="$providerName Static Analysis"
  planLabel=$(GetPlanLabelByProjectAndName "CA" "$planSearchTerm" "$atlassianUserName" "$atlassianPassword")
